...
knockOutControllers.controller('knockOutControllers', ['$scope',
    function($scope) {
        $scope.teams = shuffleArray(getTeams());
        var numOfAllTeams = $scope.teams.length;
...
function displayGoals(teams, round, gap, col){
    ...
    teams.forEach(function(team){
        ...
        node.firstChild.innerHTML = `<i class="fa fa-futbol-o" aria-hidden="true"></i> ${team.name} ${  team.goal}`;
...
function createTeamButton(name, parent){
    ...
    teamNode.innerHTML = `<i class="fa fa-futbol-o" aria-hidden="true"></i> ${name}`;
...
function displayPlayersDiv(event) {
    ...
    let players = generateFullNameForPlayers(team.players);
    players.forEach(function(player){
        node.childNodes[2].innerHTML += `<p>${player}</p>`;
    })
}
...
function generateFullNameForPlayers(players){
    return players.map(x => x.name.first[0].toUpperCase() + ". " + x.name.last[0].toUpperCase() + x.name.last.substr(1));
}
